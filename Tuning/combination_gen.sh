...
queue $(N) ' > $normal_file_sub
		echo "condor_submit $normal_file_sub & sleep 10" >> $run_all_path # append no run_all.sh
...
queue $(N) ' > $ws_file_sub
		echo "condor_submit $ws_file_sub & sleep 10" >> $run_all_path # append no run_all.sh
	done
done
